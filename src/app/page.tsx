...
import { UpdatesCta } from "@/components/landing/UpdatesCta";
...
export default async function Home() {
  const availability = await getEffectiveRegistrationAvailability();
  const paymentSnapshot = availability.snapshot;
  const registrationAvailable = availability.available;
  const manuallyClosed = availability.manuallyClosed;
  ...
  return <div id="top" className="landing-shell"><SitePreloader /><LandingHeader registrationAvailable={registrationAvailable} manuallyClosed={manuallyClosed} /><main>
    <CinematicHero registrationAvailable={registrationAvailable} manuallyClosed={manuallyClosed} />
    <section className="landing-association" aria-label="Event association"><Container><div className="association-lockup association-lockup--branded"><div className="association-lockup__logos"><Image loading="lazy" src="/images/logo2.png" alt="Illuminate 2026 logo" width={188} height={68} className="association-lockup__illuminate" /><span aria-hidden className="association-lockup__connector">·</span><Image loading="lazy" src="/images/ecell-logo.png" alt="E-Cell MET logo" width={72} height={72} className="association-lockup__met" /><span aria-hidden className="association-lockup__connector">·</span><Image loading="lazy" src="/images/e-cell-iitb.png" alt="E-Cell IIT Bombay logo" width={54} height={62} className="association-lockup__iitb" /></div><div className="association-lockup__details"><span className="association-lockup__eyebrow">Illuminate 2026</span><strong className="association-lockup__title">Hosted by {event.organizer.name}</strong><p>{event.association.label}</p></div></div></Container></section>
    ...
    <SupportContact />
    <FinalCta cta={cta} manuallyClosed={manuallyClosed} />
  </main><SiteFooter /></div>;
}
