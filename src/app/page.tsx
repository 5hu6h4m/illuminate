...
export default function Home() {
  ...
  return <div id="top" className="landing-shell"><SitePreloader /><LandingHeader registrationAvailable={registrationAvailable} /><main>
    ...
    <section className="landing-section landing-value"><Container><div className="value-grid"><div><p className="text-eyebrow">Workshop registration</p><h2 className="text-section-title">Simple, transparent pricing.</h2></div><div className="value-grid__body"><p>Registration gives you access to the Illuminate workshop.</p><p className="text-text-muted">The amount shown during registration is determined from the active registration tier.</p><div className="value-grid__rule" /><p className="text-text-muted">Registration fees are non-refundable once payment has been verified. <Link href="/refunds">Read the refund policy</Link>.</p></div><div className="price-cards"><div className={`price-card${pricingTier === "early_bird" ? " is-featured" : ""}`}><p className="price-card__tier">Early Bird{pricingTier === "early_bird" ? <span className="price-card__badge">Current</span> : null}</p><p className="price-card__amount">₹{event.fee.pricing.earlyBirdAmount.toLocaleString("en-IN")}</p><p className="price-card__note">Available during the Early Bird registration window.</p></div><div className={`price-card${pricingTier === "regular" ? " is-featured" : ""}`}><p className="price-card__tier">Regular{pricingTier === "regular" ? <span className="price-card__badge">Current</span> : null}</p><p className="price-card__amount">₹{event.fee.pricing.regularAmount.toLocaleString("en-IN")}</p><p className="price-card__note">Applies after the Early Bird window ends.</p></div></div></div></Container></section>
    <RegistrationProcess />
