...
import { buildUpiUri, getConfirmedPaymentSnapshot, isDevE2EPreviewEnabled, hashParticipantAccessToken, isValidParticipantAccessToken } from "@/lib/payment";
...
export async function GET(_: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  if (!registration || (registration.isTest ? !isDevE2EPreviewEnabled() : !getConfirmedPaymentSnapshot())) return new Response("Not found", { status: 404 });
